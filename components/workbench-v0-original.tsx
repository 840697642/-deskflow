...
function NavRail({ active, onSelect }: { active: string; onSelect: (id: string) => void }) {
  const online = MOCK_SERVICES.filter((s) => s.status === 'online').length
  return (
        ...
        <div className="rounded-lg bg-muted p-4">
          <div className="flex items-center justify-between gap-2 text-[11px]">
            ...
            <span className="font-medium tabular-nums text-foreground">
              {online} / {MOCK_SERVICES.length} 在线
            </span>
          </div>
...
type View = 'focus' | 'queue' | 'plan' | 'knowledge' | 'services' | 'projects'
...
function FocusView({
  ...
  const attentionServices = MOCK_SERVICES.filter((s) => s.status === 'offline' || s.status === 'permission_required' || s.status === 'degraded')
              ...
              {attentionServices.map((s) => {
                ...
                return (
                  <li key={s.id} className="flex min-h-16 items-center gap-4 px-6 py-3 transition-colors duration-150 hover:bg-surface-raised">
                    ...
                    {s.actionLabel ? (
                      <Button variant={s.status === 'permission_required' ? 'primary' : 'outline'}>
...
export default function Workbench() {
  ...
  const [selectedId, setSelectedId] = useState<string | undefined>('job-1042')
  ...
  const [plan, setPlan] = useState<PlanCard[]>(MOCK_PLAN)
  ...
  const movePlan = (id: string, to: PlanColumn) => {
    const target = plan.find((c) => c.id === id)
    setPlan((prev) => prev.map((c) => (c.id === id ? { ...c, column: to } : c)))
    if (target) pushToast({ tone: 'info', title: `已移到「${PLAN_COLUMNS.find((c) => c.id === to)?.label}」`, description: target.title })
  }
  ...
  // 本地模拟操作；真实实现请替换为 API 调用
  const applyAction = (id: string, action: JobAction) => {
    setJobs((prev) =>
      prev.map((j) => {
        if (j.id !== id) return j
        switch (action) {
          case 'pause':
            return { ...j, status: 'paused', heartbeat: '刚刚', heartbeatStale: false, note: '由用户手动暂停。' }
          case 'resume':
            return { ...j, status: 'running', heartbeat: '刚刚', heartbeatStale: false, note: undefined }
          case 'retry':
            return { ...j, status: 'running', progress: 0, step: '重新排队执行', heartbeat: '刚刚', note: undefined }
          case 'cancel':
            return { ...j, status: 'failed', step: '已由用户取消', heartbeat: '刚刚', note: '任务已取消，可重试。' }
          case 'authorize':
          case 'input':
            return { ...j, status: 'running', heartbeat: '刚刚', note: undefined }
          default:
            return j
        }
      }),
    ...
    switch (action) {
      case 'cancel':
        ...
        setConfirm({
          ...
          confirmLabel: '确认取消',
          onConfirm: () => {
            applyAction(id, 'cancel')
            pushToast({ tone: 'warning', title: '任务已取消', description: job.name })
          },
        })
        return
      case 'logs':
      ...
      case 'pause':
        ...
        pushToast({ tone: 'info', title: '任务已暂停', description: job.name })
        return
      case 'resume':
        applyAction(id, action)
        pushToast({ tone: 'success', title: '任务已继续', description: job.name })
        return
      case 'retry':
        applyAction(id, action)
        pushToast({ tone: 'success', title: '已重新排队执行', description: job.name })
        return
      case 'authorize':
        applyAction(id, action)
        pushToast({ tone: 'success', title: '授权成功', description: '任务将自动继续。' })
        return
      case 'input':
        applyAction(id, action)
        pushToast({ tone: 'success', title: '已提交输入', description: '任务已恢复运行。' })
        return
    }
  }
              ...
              {view === 'services' && (
                <section aria-label="服务状态" className="overflow-hidden rounded-lg bg-card shadow-sm">
                  <ul className="grid grid-cols-1 divide-y divide-border/60 md:grid-cols-2 md:divide-y-0 md:[&>li:nth-child(n+3)]:border-t md:[&>li:nth-child(odd)]:border-r md:[&>li]:border-border/60">
                    {MOCK_SERVICES.map((s) => (
                      <ServiceItem key={s.id} service={s} onAction={() => pushToast({ tone: 'info', title: `正在${s.actionLabel}`, description: s.name })} />
                    ))}
                  </ul>
                </section>
              )}
